...
b0 = 0.2
b = np.array([0.05,0.15,0.1,0.07])

k = np.array([0.115525,0.462098,0.12,0.3])
drugs = ['Propofol','Levetiracetam','Midazolam','Valproate']
D = np.random.binomial(1, 0.05,(48,len(drugs))) 
...
ax[0].plot(np.arange(0,T),p, c='#ff7f0e',label='Mean',alpha=0)
ax[0].fill_between(x=np.arange(2,T),y1=p[2:]+np.random.normal(0.05,0.0025,size=len(p)-2),y2=p[2:]-np.random.normal(0.05,0.0025,size=len(p)-2),alpha=0*0.25,color='red')
ax[0].legend()
ax[0].set_title('Sz Burden')
