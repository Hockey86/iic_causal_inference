...
for posterior_stat in ['mean', 'std']:
    ...
    data['alpha1'] = [df_params['alpha1[%d]'%i] for i in range(1,N+1)]
    data['alpha2'] = [df_params['alpha2[%d]'%i] for i in range(1,N+1)]

    # beta
    for d in range(1,Ndrug+1):
        data['beta[%d]'%d] = [df_params['beta[%d,%d]'%(i,d)] for i in range(1,N+1)]

    # for patients with some drug, make b NaN for the missing drug
    for i in range(N):
        Dmax = Ddose[i].max(axis=0)
        for d in range(Ndrug):
            if Dmax[d]==0:
                data['beta[%d]'%(d+1,)][i] = np.nan

    # save
    df = pd.DataFrame(data=data)
    df_C = pd.DataFrame(data=C, columns=Cname)
    df = pd.concat([df, df_C], axis=1)
    df = df.rename(columns={'beta[%d]'%(i+1,):'b[%s]'%Dname[i] for i in range(len(Dname))})
