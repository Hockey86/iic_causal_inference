...
with open('/data/IIC-Causality/mycode/aim1/simulator/model_fit2.pkl', 'rb') as f:
    model, fit = pickle.load(f)
with open('/data/IIC-Causality/mycode/aim1/simulator/results.pickle', 'rb') as ff:
    res  = pickle.load(ff)
Ep_sim = res['Ep_sim'][0]
sids = res['sids']
...
tostudy_Dnames = ['lacosamide', 'levetiracetam', 'midazolam', 
...
for si, sid in enumerate(tqdm(sids)):
    ...
    #iic_sim = res_sim['Esim'][:,0]
    iic_sim = Ep_sim[si]#.mean(axis=0)
    
    iic_sim = np.repeat(iic_sim, 900)
    if len(iic_sim)>=len(human_label):
        iic_sim = iic_sim[:len(human_label)]
    else:
        iic_sim = np.r_[iic_sim, np.zeros(len(human_label)-len(iic_sim))+np.nan]
