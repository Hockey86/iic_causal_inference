...
max_iter = [1000,1000]
metrics = ['stRMSE', 'WAIC', 'loglikelihood', 'CI95 Coverage']#, 'WAIC']
W = 300
random_state = 2020
...
for model, metric in product(models, metrics):
    ...
    if metric in ['loglikelihood', 'CI95 Coverage']:
        ...
        perf[(model, 'stRMSE')] = simulator.score(
                                [x[AR_T0:] for x in Pobs],
                                [x[:,AR_T0:] for x in Psim],
                                method=metric, Ts_stRMSE=Ts)
        perf['Ts_stRMSE'] = Ts
        for ii, t in enumerate(Ts):
            perf_ = np.nanmean(perf[(model, 'stRMSE')][ii], axis=0)
