...
if __name__=='__main__':
    ...
    for sid in tqdm(common_sids):
        ...
        # convert into iic vs non iic
        nan_ids = np.isnan(human_iic)
        human_iic = np.in1d(human_iic, iic_labels).astype(float)
        human_iic[nan_ids] = np.nan
        nan_ids = np.isnan(cnn_iic)
        cnn_iic = np.in1d(cnn_iic, iic_labels).astype(float)
        cnn_iic[nan_ids] = np.nan
    ...
    for si, sid in enumerate(tqdm(common_sids)):
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig('human_vs_CNN_IIC_IICvsnonIIC/%s.png'%sid)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('CNN_vs_human_IIC_cf_IICvsnonIIC.png', bbox_inches='tight', pad_inches=0.05)
    ...
    plt.tight_layout()
    #plt.show()
    plt.savefig('CNN_vs_human_IIC_cf_smoothed_IICvsnonIIC.png', bbox_inches='tight', pad_inches=0.05)
