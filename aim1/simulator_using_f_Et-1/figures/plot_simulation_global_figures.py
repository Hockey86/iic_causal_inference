...
for model in models:
    ...
    for si, sid in enumerate(tqdm(sids)):
        ...
        gs_ax3 = gs[2,0].subgridspec(len(has_drug_ids), 1, hspace=0.02)
        for axi, di in enumerate(has_drug_ids):
            ax3 = fig.add_subplot(gs_ax3[axi,0])
            ax3.set_title(Dnames[di])
            ax3.imshow(Dscaled[si][:,di].reshape(1,-1), aspect='auto',
                       extent=(tt.min(), tt.max(), 0,1),
                       vmin=0, vmax=Dmax[di])
